...
#else
static inline uint32_t set_swi_errno(struct CPUARMState *env, uint32_t code)
{
    return code;
}
...
static void arm_semi_cb(struct CPUARMState *env, target_ulong ret, target_ulong err)
{
#ifdef CONFIG_USER_ONLY
...
static void arm_semi_flen_cb(struct CPUARMState *env, target_ulong ret, target_ulong err)
{
...
uint32_t do_arm_semihosting(struct CPUARMState *env)
{
    target_ulong args;
    char * s;
    int nr;
